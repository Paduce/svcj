...
class BaseSVModel(nn.Module):
    ...
    def _handle_input(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Expected input: (B, 1, L) from dataset
        Returns: (B, 1, L) for conv layers
        """
        if x.dim() == 2:
            # (B, L) → (B, 1, L)
            x = x.unsqueeze(1)
    ...
    def _compute_mu_features(self, x: torch.Tensor, mu_mode: str) -> torch.Tensor:
        ...
        """
        # Remove channel dimension for statistics computation
...
class TCNRegressor(BaseSVModel):
    ...
    def __init__(self, config: ModelConfig, n_levels: int = 4, k: int = 3):
        ...
        for i in range(n_levels):
            ...
            tcn_block = _tcn_block(in_ch, out_ch, k, dilation, config.dropout_rate)
            tcn_layers.append(tcn_block)
...
class TransformerRegressor(BaseSVModel):
    """
    Transformer-based regressor for SV parameter estimation.
    Uses positional encoding and multi-head attention.
    """
    
    def __init__(self, config: ModelConfig, n_heads: int = 8, n_layers: int = 4):
        super().__init__(config)
        
        self.d_model = self.config.hidden_dim
    ...
    def forward(self, x: torch.Tensor, mu_mode: str = "raw") -> torch.Tensor:
        ...
        B, C, L = original_input.shape
        
        # Reshape for transformer: (B, L, C)
        x_for_transformer = original_input.transpose(1, 2)  # (B, L, 1)
        
        # Project to model dimension
        x_proj = self.input_proj(x_for_transformer)  # (B, L, d_model)
        
        # Add positional encoding
        if L <= self.pos_encoding.size(0):
            x_proj = x_proj + self.pos_encoding[:L]
        else:
            # Interpolate positional encoding for longer sequences
            pos_enc = F.interpolate(
                self.pos_encoding[:, None, :].transpose(1, 2), 
                size=L, mode='linear', align_corners=False
            ).transpose(1, 2).squeeze(1)
            x_proj = x_proj + pos_enc
        
        # Transformer encoding
        transformer_out = self.transformer(x_proj)  # (B, L, d_model)
        
        # Global pooling
        transformer_out = transformer_out.transpose(1, 2)  # (B, d_model, L)
        features = self.global_pool(transformer_out).flatten(1)  # (B, d_model)
...
def build_model(
    ...
    **kwargs
    ...
    kind : str, default="cnn"
        Model architecture: "cnn", "cnn_ficura", "tcn", "transformer"
...
if __name__ == "__main__":
    ...
    config = ModelConfig(n_params=7, model_type="svcj", dropout_rate=0.1)
    
    for arch in ["cnn", "cnn_ficura", "tcn", "transformer"]:
        ...
        try:
            ...
            # Test consistency
            consistency = test_model_consistency(model, seq_len=252)
